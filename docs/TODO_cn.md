...
- 真实长期样本质量：daemon replay 指纹持久化、键鼠输入学习分析、现场教学自动入库和 synthetic proposal/apply smoke 已完成；仍需人工/HID 真实样本对照验证 profile 是否持续收敛。
...
- 回放级 `compact trace` 指纹：daemon 持久化、键鼠输入学习分析和现场教学已落地；下一步是真实样本回归
