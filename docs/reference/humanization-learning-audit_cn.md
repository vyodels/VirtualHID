...
VirtualHID 已具备可用于 mock recruiting workflow 后续评估的通用拟人化与学习基础：轨迹、节律、落点采样、profile 聚合、执行时 profile 应用、daemon replay 指纹持久化、长期分析入口、replay-aware report 和受控 profile patch apply 都已经存在。当前缺口不再是“没有拟人化/没有学习”，而是真实长期人工/HID 样本质量还没有闭环验收，`imePinyin` 逐字中文输入也尚未实现。
