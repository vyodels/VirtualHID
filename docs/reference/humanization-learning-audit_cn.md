...
VirtualHID 已具备可用于 mock recruiting workflow 后续评估的通用拟人化与学习基础：轨迹、节律、落点采样、键鼠输入学习分析、现场教学、profile 聚合、执行时 profile 应用、daemon replay 指纹持久化、长期分析入口、replay-aware report 和受控 profile patch apply 都已经存在。当前缺口不再是“没有拟人化/没有学习”，而是真实长期人工/HID 样本质量还没有闭环验收，`imePinyin` 逐字中文输入也尚未实现。
...
- 现场教学：`learning.teaching.start/stop` 支持给一段真实练习窗口打范围标签，管理中心可用 HUD 标出起点、目标点和动作说明；样本会实时自动进入 `ProfileStore`，stop 只结束教学窗口并触发模板重建，不作为手动保存入口。`learning.session.start/stop` 仅作为兼容别名保留。
...
- `VirtualHID.app` 管理中心已增加中文学习控制面：开启/关闭键鼠输入学习分析、开始/结束现场教学、用 HUD 标记教学起点/目标点/动作说明、展示系统事件监听状态、实时原始事件、动作片段、历史片段，并单独提供“能力模板”和“效果分析”页面。效果分析通过安全 dry-run 的 `learning.demo.run` 触发，让 HUD 分步展示 VirtualHID planned events 产生的未使用模板 / 使用模板轨迹差异，不真实点击页面。
