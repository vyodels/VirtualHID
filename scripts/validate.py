...
def main():
    ...
    try:
        ...
        artifacts["active_stdout"] = run_injector(binary, "mouse_move_click_active,mouse_drag_active,keyboard_type_active")
        time.sleep(1.2)
        ...
        artifacts["blur_stdout"] = run_injector(binary, "mouse_move_click_blur,keyboard_type_blur")
        time.sleep(1.2)
        artifacts["blur_frontmost_after_injection"] = frontmost_app()
        artifacts["textedit_text_after_blur_injection"] = get_textedit_text()
        osa('tell application "Safari" to activate')
        time.sleep(1.2)
        artifacts["blur_report_after"] = read_report(wait_seconds=3.0)

        write_json(RESULTS / "validation-artifacts.json", artifacts)
        print(json.dumps({"status": "ok", "results": str(RESULTS / "validation-artifacts.json")}, ensure_ascii=False))
    finally:
        server.terminate()
